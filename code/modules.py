...
class BiDirAttn(object): 
    ...
    def build_graph(self, values, values_mask, keys):
        ...
        Outputs:
          output: Tensor shape (batch_size, num_keys, hidden_size * 8).
            The bidirectional attention output.
        """

        with vs.variable_scope("BiDirAttn"):
            # Compute similarity matrix
            w_sim = tf.get_variable("w_sim", shape=(values.get_shape().as_list()[2] * 3, 1), dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer()) # (6 * hidden_size, 1)
            ...
            reshaped_keys = tf.reshape(keys, [-1, keys.get_shape().as_list()[2]]) # (batch_size * num_keys, 2 * hidden_size)
            reshaped_values = tf.reshape(values, [-1, values.get_shape().as_list()[2]]) # (batch_size * num_values, 2 * hidden_size)
            reshaped_sim_scores = tf.reshape(sim_scores, [-1, keys.get_shape().as_list()[2]]) # (batch_size * num_keys * num_values, 2 * hidden_size)
            
            reshaped_keys_weighted = tf.matmul(reshaped_keys, w_sim[:keys.get_shape().as_list()[2], :]) # (batch_size * num_keys, 1)
            reshaped_values_weighted = tf.matmul(reshaped_values, w_sim[values.get_shape().as_list()[2]:values.get_shape().as_list()[2] * 2, :]) # (batch_size * num_values, 1)
            reshaped_sim_scores_weighted = tf.matmul(reshaped_sim_scores, w_sim[values.get_shape().as_list()[2] * 2:values.get_shape().as_list()[2] * 3, :]) # (batch_size * num_keys * num_values, 1)

            keys_weighted = tf.reshape(reshaped_keys_weighted, [-1, keys.shape[1], 1]) # (batch_size, num_keys, 1)
            values_weighted = tf.reshape(reshaped_values_weighted, [-1, 1, values.shape[1]]) # (batch_size, 1, num_values)
            sim_scores_weighted = tf.reshape(reshaped_sim_scores_weighted, [-1, keys.shape[1], values.shape[1]]) # (batch_size, num_keys, num_values)
            
            sim = keys_weighted + values_weighted + sim_scores_weighted # (batch_size, num_keys, num_values)
